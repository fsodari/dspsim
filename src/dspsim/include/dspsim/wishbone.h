...
namespace dspsim
{
    ...
    public:
        ...
        std::vector<DT> rx_data(int clear = -1);
        ...
        void write_command(int address, DT data);
        void write_command(int start_address, std::vector<DT> &data);
        // Write a map/dict of addresses and data.
        void write_command(std::map<int, DT> &data);

        // Send a write command and wait until it's done.
        void write_block(int address, DT data, int timeout = -1);
        void write_block(int start_address, std::vector<DT> &data, int timeout = -1);
        void write_block(std::map<int, DT> &data, int timeout = -1);
        void write_blockf(int address, double data, int timeout = -1);
        void write_blockf(int start_address, std::vector<double> &data, int timeout = -1);
        void write_blockf(std::map<int, double> &data, int timeout = -1);
